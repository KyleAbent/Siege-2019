...
--]]
    function(bot, brain)
        ...
        if order ~= nil then
            ...
            if not isExo and target ~= ni and ( order:GetType() == kTechId.Construct or order:GetType() == kTechId.Build ) then
                ...
                if numOthers and numOthers >= 1 then --hm?
                    weight = 0.0
                else
                    weight = 2.0
                end
    ...
    function(bot, brain)
        ...
        local availableWeapons = { }         --write for fl to priotize structuresd over players
        local weaponTechs = { [kTechId.ShotgunTech] = kTechId.Shotgun, [kTechId.AdvancedWeaponry] = kTechId.GrenadeLauncher, [kTechId.AdvancedWeaponry] = kTechId.Flamethrower, [kTechId.HeavyMachineGunTech] = kTechId.HeavyMachineGun }
        ...
        local wantNewWeapon = (not availableWeapons[bestWeaponTechId] and choice) and true or false
            ...
            perform = function(move)
                ...
                if armory ~= nil then
                    ...
                    if touchDist > 1.5 then
                        ...
                        bot:GetMotion():SetDesiredViewTarget( armory:GetEngagementPoint() )
                        bot:GetMotion():SetDesiredMoveTarget( nil )
                        bot:GetPlayer():ProcessBuyAction({ choice })
